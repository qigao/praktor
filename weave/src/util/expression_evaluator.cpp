...
struct VariableNode : ASTNode
{
  ...
  LiteralValue evaluate(const WorkflowContext& context) const override
  {
    ...
    std::cout << "DEBUG: Variable '" << trimmed_name << "' not found in context"
              << std::endl;
    LOG_WARNING("Expression variable '" + trimmed_name
                + "' not found in context, evaluating as empty string.");
    return {};
  }
};
...
struct op_eq_tail : seq<op_eq, primary>
{
};

struct op_neq_tail : seq<op_neq, primary>
{
};

struct comparison_expr : seq<primary, star<sor<op_eq_tail, op_neq_tail>>>
{
};
...
struct main : seq<ws, expression, ws, eof>
{
};
