...
void SystemInfoProvider::initializeLoop() {
    ...
    if (!loop_) {
        ...
        if (result != 0) {
            delete loop_;
            loop_ = nullptr;
            LOG_ERROR("Failed to initialize libuv loop: " + uvErrorToString(result));
            return;
        }
        owns_loop_ = true;
    }
...
std::string SystemInfoProvider::getOSName() const {
    ...
    if (result != 0) {
        LOG_WARNING("Failed to get OS name via libuv: " + uvErrorToString(result));
        return "unknown";
    }
...
std::string SystemInfoProvider::getOSVersion() const {
    ...
    int result = uv_os_uname(&utsname);
    if (result != 0) {
        LOG_WARNING("Failed to get OS version via libuv: " + uvErrorToString(result));
        return "unknown";
    }
...
std::string SystemInfoProvider::getArchitecture() const {
    ...
    int result = uv_os_uname(&utsname);
    if (result != 0) {
        LOG_WARNING("Failed to get architecture via libuv: " + uvErrorToString(result));
        return "unknown";
    }
...
std::string SystemInfoProvider::getUsername() const {
   ...
   int result = uv_os_get_passwd(&pwd); // Pass the address of the pointer
    if (result != 0) {
        LOG_WARNING("Failed to get username via libuv: " + uvErrorToString(result));
        return "unknown";
    }
...
std::string SystemInfoProvider::getHostname() const {
    ...
    if (result != 0) {
        LOG_WARNING("Failed to get hostname via libuv: " + uvErrorToString(result));
        return "unknown";
    }
...
SystemInfoProvider::CPUInfo SystemInfoProvider::getCPUInfo() const {
    ...
    if (result != 0) {
        LOG_WARNING("Failed to get CPU info via libuv: " + uvErrorToString(result));
        return info;
    }
...
std::vector<SystemInfoProvider::NetworkInterface> SystemInfoProvider::getNetworkInterfaces() const {
    ...
    if (result != 0) {
        LOG_WARNING("Failed to get network interfaces via libuv: " + uvErrorToString(result));
        return interfaces;
    }
...
std::unordered_map<std::string, std::string> SystemInfoProvider::getAllSystemProperties() const {
    ...
    // Basic system info
    props["WEAVE_OS_NAME"] = getOSName();
    props["WEAVE_OS_VERSION"] = getOSVersion();
    props["WEAVE_ARCH"] = getArchitecture();
    props["WEAVE_SHELL"] = getShellName();
    props["WEAVE_USER"] = getUsername();
    props["WEAVE_HOST"] = getHostname();
...
std::unordered_map<std::string, std::string> SystemInfoProvider::getEnvironmentVariables() const {
    ...
    if (result != 0) {
        LOG_WARNING("Failed to get environment variables via libuv: " + uvErrorToString(result));
        return env_vars;
    }
...
std::unordered_map<std::string, std::string> SystemInfoProvider::getExtendedSystemProperties() const {
    ...
    for (const auto& pair : env_vars) {
        props["WEAVE_ENV_" + pair.first] = pair.second;
    }

    // CPU information
    auto cpu_info = getCPUInfo();
    props["WEAVE_CPU_MODEL"] = cpu_info.model;
    props["WEAVE_CPU_CORES"] = std::to_string(cpu_info.core_count);
    props["WEAVE_CPU_SPEED_MHZ"] = std::to_string(static_cast<int>(cpu_info.speed_mhz));

    // Memory information
    auto mem_info = getMemoryInfo();
    props["WEAVE_MEMORY_TOTAL_GB"] = std::to_string(mem_info.total_memory / (1024 * 1024 * 1024));
    props["WEAVE_MEMORY_FREE_GB"] = std::to_string(mem_info.free_memory / (1024 * 1024 * 1024));

    // Load average (Unix-like systems)
    auto load = getLoadAverage();
    props["WEAVE_LOAD_1MIN"] = std::to_string(load.one_minute);
    props["WEAVE_LOAD_5MIN"] = std::to_string(load.five_minutes);
    props["WEAVE_LOAD_15MIN"] = std::to_string(load.fifteen_minutes);

    // Network interfaces count
    auto interfaces = getNetworkInterfaces();
    props["WEAVE_NETWORK_INTERFACES"] = std::to_string(interfaces.size());
