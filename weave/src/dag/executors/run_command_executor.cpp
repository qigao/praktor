...
TaskResult RunCommandExecutor::execute(const Task& task,
                                       WorkflowContext& context)
{
  LOG_INFO("Executing run_command: " + task.name);
...
TaskResult RunCommandExecutor::executeProcess(const RunCommandParams& params,
                                              const WorkflowContext& context)
{
  ...
  if (!params.environment.empty()) {
    for (const auto& [key, value] : params.environment) {
      env_vars.push_back(key + "=" + substituteVariables(value, context));
    }
    proc_context.env_ptr.reserve(env_vars.size() + 1);
    for (auto& var : env_vars) {
      proc_context.env_ptr.push_back(&var[0]);
    }
    proc_context.env_ptr.push_back(NULL);
  }
  ...
  proc_context.options.env =
      params.environment.empty() ? NULL : proc_context.env_ptr.data();
  ...
  if (std::holds_alternative<std::string>(params.command)) {
    ...
    LOG_DEBUG("Command: " + full_command + " (CWD: " + actual_cwd + ")");
  } else {
    if (!command_parts.empty()) {
        LOG_DEBUG("Command (list): " + command_parts[0] + " (CWD: " + actual_cwd + ")");
    } else {
        LOG_DEBUG("Command (list): <empty> (CWD: " + actual_cwd + ")");
    }
  }
  ...
  if (!result.success) {
    ...
    LOG_ERROR("Command failed. Stdout: " + result.stdout_data + " Stderr: " + result.stderr_data);
  } else {
    LOG_DEBUG("Command succeeded. Stdout: " + result.stdout_data + " Stderr: " + result.stderr_data);
  }

  return result;
}

void RunCommandExecutor::applyOutputs(const Task& task,
                                      const TaskResult& result,
                                      WorkflowContext& context)
{
  // Apply output variable bindings
  if (!task.outputs.stdout_to_variable.empty()) {
    context.setValue(task.outputs.stdout_to_variable, result.stdout_data);
  }
  if (!task.outputs.stderr_to_variable.empty()) {
    context.setValue(task.outputs.stderr_to_variable, result.stderr_data);
  }
  if (!task.outputs.exit_code_to_variable.empty()) {
    context.setValue(task.outputs.exit_code_to_variable,
                     std::to_string(result.exit_code));
  }

  // Handle JSON output parsing
  if (!task.outputs.output_json_to_variable.empty()) {
    try {
      WorkflowValue parsed_json = jsoncons::json::parse(result.stdout_data);
      context.setValue(task.outputs.output_json_to_variable, parsed_json);
    } catch (const jsoncons::json_exception& e) {
      throw std::runtime_error("Failed to parse stdout as JSON for '"
                               + task.name + "': " + e.what());
    }
  }
}
...
{
  return std::make_unique<RunCommandExecutor>();
}
