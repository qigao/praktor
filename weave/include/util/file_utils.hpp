...
namespace FileUtils {
    ...
    inline std::string uvErrorToString(int error_code) {
    ...
    inline std::string readFile(std::string const& filePath) {
        if (!fileExists(filePath)) { throw FileException("File not found: " + filePath); }
    ...
    inline bool writeFile(std::string const& filePath, std::string const& content) {
        uv_fs_t open_req;
        int r = uv_fs_open(uv_default_loop(), &open_req, filePath.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0644, NULL);
        if (r < 0) {
            uv_fs_req_cleanup(&open_req);
            return false;
        }
