...
std::unordered_map<std::string, std::string> SystemInfoProvider::getAllSystemProperties() const {
    ...
    // Basic system info
    props["PRAKTER_OS_NAME"] = getOSName();
    props["PRAKTER_OS_VERSION"] = getOSVersion();
    props["PRAKTER_ARCH"] = getArchitecture();
    props["PRAKTER_SHELL"] = getShellName();
    props["PRAKTER_USER"] = getUsername();
    props["PRAKTER_HOST"] = getHostname();
...
std::unordered_map<std::string, std::string> SystemInfoProvider::getExtendedSystemProperties() const {
    ...
    for (const auto& pair : env_vars) {
        props["PRAKTER_ENV_" + pair.first] = pair.second;
    }

    // CPU information
    auto cpu_info = getCPUInfo();
    props["PRAKTER_CPU_MODEL"] = cpu_info.model;
    props["PRAKTER_CPU_CORES"] = std::to_string(cpu_info.core_count);
    props["PRAKTER_CPU_SPEED_MHZ"] = std::to_string(static_cast<int>(cpu_info.speed_mhz));

    // Memory information
    auto mem_info = getMemoryInfo();
    props["PRAKTER_MEMORY_TOTAL_GB"] = std::to_string(mem_info.total_memory / (1024 * 1024 * 1024));
    props["PRAKTER_MEMORY_FREE_GB"] = std::to_string(mem_info.free_memory / (1024 * 1024 * 1024));

    // Load average (Unix-like systems)
    auto load = getLoadAverage();
    props["PRAKTER_LOAD_1MIN"] = std::to_string(load.one_minute);
    props["PRAKTER_LOAD_5MIN"] = std::to_string(load.five_minutes);
    props["PRAKTER_LOAD_15MIN"] = std::to_string(load.fifteen_minutes);

    // Network interfaces count
    auto interfaces = getNetworkInterfaces();
    props["PRAKTER_NETWORK_INTERFACES"] = std::to_string(interfaces.size());
