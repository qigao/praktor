                        ...
                        const std::unordered_map<std::string, std::string>& headers)
{
    ...
    HINTERNET hSession = WinHttpOpen(L"Prakter/1.0",
                                          ...
                                          const std::unordered_map<std::string, std::string>& environment)
{
    std::visit([this, &context, &environment](auto&& arg) {
        ...
        } else if constexpr (std::is_same_v<T, PrakterNotifyTrigger>) {
            executePrakterNotify(arg, context, environment);
        }
    }, action);
}
...
void TriggerExecutor::executePrakterNotify(const PrakterNotifyTrigger& trigger,
                                         WorkflowContext& context,
                                         const std::unordered_map<std::string, std::string>& environment)
{
    std::string message = substituteVariables(trigger.message, context);

    // Prefer explicit PRAKTER_WEBHOOK, fallback to PRAKTER_NOTIFY_URL
    std::string webhook;
    auto it = environment.find("PRAKTER_WEBHOOK");
    if (it != environment.end()) {
        webhook = it->second;
    } else {
        it = environment.find("PRAKTER_NOTIFY_URL");
        if (it != environment.end()) webhook = it->second;
    }

    if (webhook.empty()) {
        logi("@prakter {}", message);
        return;
    }
