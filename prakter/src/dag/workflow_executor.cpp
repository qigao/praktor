...
WorkflowExecutor::WorkflowExecutor(DependencyGraph<Task>& graph,
    ...
    , thread_pool_(num_threads > 1 ? std::make_unique<ThreadPool>(num_threads) : nullptr)
{

    // Initialize executor registry
    executors_[TaskAction::RunCommand] = Prakter::Execution::createCommandExecutor();
    executors_[TaskAction::Script] = Prakter::Execution::createScriptExecutor();
    executors_[TaskAction::Uses] = Prakter::Execution::createUsesExecutor(base_environment_, num_threads);

    // DynamicTasks executor needs a callback to execute generated subtasks
    auto dynamic_executor = Prakter::Execution::createDynamicTasksExecutor();
    auto* dt_ptr = static_cast<Prakter::Execution::DynamicTasksExecutor*>(dynamic_executor.get());
...
void WorkflowExecutor::execute(WorkflowContext& context, std::optional<std::string> alias)
{
    ...
    context.setValue("workflow_status", (state.workflow_failed && !state.should_stop) || state.should_stop ? "failed" : "success");
...
{
    if (workflow_path.empty()) return;
    auto path = std::filesystem::path(workflow_path).parent_path() / ".prakter_cache";
...
{
    if (cache_file_.empty() || cache_.empty()) return;
...
bool WorkflowExecutor::checkSkipTask(const Task& task, WorkflowContext& context)
{
    ...
    for (const auto& src : task.sources) {
        ...
        auto current_hash = Prakter::Util::computeFileHash(path);
...
void WorkflowExecutor::updateTaskCache(const Task& task, WorkflowContext& context)
{
    ...
    for (const auto& src : task.sources) {
        auto path = resolveRelativePath(task.source_path, src);
        state.source_hashes[path.string()] = Prakter::Util::computeFileHash(path);
    }
                                    ...
                                    WorkflowContext& context) const
{
    ...
    try {
        return Prakter::Expressions::ExpressionEvaluator{}.evaluateAsBool(*task.when, context);
    } catch (const std::exception& e) {
    ...
    WorkflowContext& context) const
{
    ...
    for (const auto& file : task.dot_env) {
        ...
        try {
            auto parsed = Prakter::util::parseDotEnvFile(env_path);
