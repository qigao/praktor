...
PrakterNotifyTrigger parse_prakter_notify_trigger(const ryml::ConstNodeRef& node) {
    PrakterNotifyTrigger trigger;
    if (node.is_val()) {
        std::string value;
        node >> value;
        std::string trimmed = trim_copy(value);
        const std::string prefix = "@prakter";
        ...
        if (!node.has_child("message")) {
            throw_parse_error(node, "prakter trigger requires 'message'");
        }
        node["message"] >> trigger.message;
    } else {
        throw_parse_error(node, "prakter trigger must be a string or map");
    }

    if (trigger.message.empty()) {
        throw_parse_error(node, "prakter trigger 'message' cannot be empty");
    }
    return trigger;
...
TriggerAction parse_trigger_action_node(const ryml::ConstNodeRef& node) {
    if (node.is_val()) {
        ...
        if (trimmed.rfind("@prakter", 0) == 0) {
            return TriggerAction{parse_prakter_notify_trigger(node)};
        }
    ...
    for (const auto& child : node) {
        ...
        if (key == "prakter" || key == "@prakter") {
            return TriggerAction{parse_prakter_notify_trigger(child)};
        }
