...
TaskResult ScriptExecutor::execute(const Task& task, WorkflowContext& context) {
    ...
    for (const auto& module_name : params.modules) {
        ...
        if (module && module->language == "javascript") {
            ...
            JSValue val = JS_Eval(ctx, source.c_str(), source.size(), module_name.c_str(), JS_EVAL_TYPE_GLOBAL);
    ...
    JSValue result_val = JS_Eval(ctx, source.c_str(), source.size(), task.name.c_str(), JS_EVAL_TYPE_GLOBAL);
...
void ScriptExecutor::applyOutputs(const Task& task, const TaskResult& result, WorkflowContext& context) {
