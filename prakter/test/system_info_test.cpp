...
using namespace Prakter::system;
...
TEST_CASE("SystemInfoProvider - All System Properties", "[system_info][libuv]") {
    ...
    SECTION("Basic Properties") {
        ...
        REQUIRE(props.find("PRAKTER_OS_NAME") != props.end());
        REQUIRE(props.find("PRAKTER_OS_VERSION") != props.end());
        REQUIRE(props.find("PRAKTER_ARCH") != props.end());
        REQUIRE(props.find("PRAKTER_SHELL") != props.end());
        REQUIRE(props.find("PRAKTER_USER") != props.end());
        REQUIRE(props.find("PRAKTER_HOST") != props.end());
    ...
    SECTION("Extended Properties") {
        ...
        REQUIRE(props.find("PRAKTER_CPU_MODEL") != props.end());
        REQUIRE(props.find("PRAKTER_CPU_CORES") != props.end());
        REQUIRE(props.find("PRAKTER_MEMORY_TOTAL_GB") != props.end());
        REQUIRE(props.find("PRAKTER_NETWORK_INTERFACES") != props.end());

        // CPU cores should be a valid number
        int cores = std::stoi(props["PRAKTER_CPU_CORES"]);
        REQUIRE(cores > 0);
        REQUIRE(cores <= 256);

        // Memory should be reasonable
        int memory_gb = std::stoi(props["PRAKTER_MEMORY_TOTAL_GB"]);
        REQUIRE(memory_gb >= 0); // Could be 0 if less than 1GB

        // Network interfaces count should be positive
        int iface_count = std::stoi(props["PRAKTER_NETWORK_INTERFACES"]);
        ...
        for (const auto& pair : env_vars) {
            std::string expected_key = "PRAKTER_ENV_" + pair.first;
...
TEST_CASE("SystemInfoProvider - Convenience Functions", "[system_info][libuv]") {
    ...
    REQUIRE_FALSE(props.empty());
    REQUIRE(props["PRAKTER_OS_NAME"] == osName);
    REQUIRE(props["PRAKTER_ARCH"] == arch);
    REQUIRE(props["PRAKTER_USER"] == user);
}
...
TEST_CASE("WorkflowContext - libuv System Variables Integration", "[workflow_context][libuv]") {
    ...
    REQUIRE(context.hasKey("PRAKTER_OS_NAME"));
    REQUIRE(context.hasKey("PRAKTER_OS_VERSION"));
    REQUIRE(context.hasKey("PRAKTER_ARCH"));
    REQUIRE(context.hasKey("PRAKTER_SHELL"));
    REQUIRE(context.hasKey("PRAKTER_USER"));
    REQUIRE(context.hasKey("PRAKTER_HOST"));

    // Values should match the libuv-based function calls
    REQUIRE(context.getVariable("PRAKTER_OS_NAME") == getOSName());
    REQUIRE(context.getVariable("PRAKTER_ARCH") == getArchitecture());
    REQUIRE(context.getVariable("PRAKTER_USER") == getUsername());
}
