...
TEST_CASE("parse @prakter trigger shorthand and mapping")
{
    ...
    auto wf = writeTempWorkflow("trigger_prakter.yml", content);
    ...
    CHECK(std::holds_alternative<PrakterNotifyTrigger>(tr.on_success[0]));
    CHECK(std::holds_alternative<PrakterNotifyTrigger>(tr.on_failure[0]));
}
