...
TEST_CASE("Execution Engine - Task Caching", "[execution][caching]") {
    ...
    fs::path cache_file = fs::absolute(".prakter_cache");
    ...
    SECTION("First run: execute and cache") {
        ...
        fs::path expected_cache = wf_path.parent_path() / ".prakter_cache";
    ...
    if (fs::exists(wf_path.parent_path() / ".prakter_cache")) fs::remove(wf_path.parent_path() / ".prakter_cache");
}
