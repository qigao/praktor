...
int main(int argc, char* argv[]) {
    CliConfig config;
    cxxopts::Options options("prakter", "Prakter - A task & workflow orchestrator by DAG");

    options.add_options()
        ("command", "Subcommand (run, init, validate, export, benchmark, visualize)", cxxopts::value<std::string>(config.command)->default_value("run"))
    ...
    try {
        ...
        if (config.showVersion) {
            std::cout << PRAKTER_PROJECT_NAME << " v" << PRAKTER_VERSION << std::endl;
            return 0;
        }

        // Subcommand dispatch
        if (config.command == "init") {
            return Prakter::Utils::PrakterInit::initialize(config.templateName) ? 0 : 1;
        }

        if (config.yamlPath.empty()) {
            std::cerr << "Error: YAML workflow file is required for command '" << config.command << "'" << std::endl;
            return 1;
        }
        ...
        if (config.command == "validate") {
            try {
                ...
                std::cout << "Validation successful: " << config.yamlPath << " is a valid Prakter workflow." << std::endl;
                return 0;
        ...
        if (config.command == "visualize") {
            ...
            try {
                auto workflow = TaskParser::parseFile(config.yamlPath);
                return Prakter::Utils::DagExporter::exportToDot(workflow, config.outputPath) ? 0 : 1;
            } catch (const std::exception& e) {
        ...
        if (config.command == "export") {
            ...
            try {
                auto workflow = TaskParser::parseFile(config.yamlPath);
                return Prakter::Utils::GithubActionsExporter::exportToYaml(workflow, config.outputPath) ? 0 : 1;
            } catch (const std::exception& e) {
        ...
        if (config.command == "run" || config.command == "benchmark") {
