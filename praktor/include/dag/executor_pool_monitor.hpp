...
public:
  static void logPoolStats()
  {
    ...
    TLOG_INFOF("Registered executor types: {}", stats.registered_types);
    TLOG_INFOF("Total cached executors: {}", stats.total_executors);
    TLOG_INFOF("Cache hits: {}", stats.cache_hits);
    TLOG_INFOF("Cache misses: {}", stats.cache_misses);
    
    if (stats.cache_hits + stats.cache_misses > 0) {
      double hit_rate = static_cast<double>(stats.cache_hits) / 
                       (stats.cache_hits + stats.cache_misses) * 100.0;
      TLOG_INFOF("Cache hit rate: {:.2f}%", hit_rate);
