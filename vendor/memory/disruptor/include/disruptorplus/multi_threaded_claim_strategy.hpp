...
namespace disruptorplus {
    ...
    /// is wanting to read.
    ///
    /// This means concurrent writers are wait-free when there is space
    /// available in the ring buffer, requiring a single atomic fetch-add
    /// operation as the only contended write operation. All other writes are to
    /// memory locations owned by a particular writer. Concurrent writers can
    /// publish items out-of-order so that one writer does not hold up other
    /// writers until the ring buffer fills up.
    ///
    /// \tparam WaitStrategy
    ...
    public:
        ...
        multi_threaded_claim_strategy(size_t bufferSize, WaitStrategy& waitStrategy) :
            m_indexMask(bufferSize - 1), m_bufferSize(bufferSize), m_waitStrategy(waitStrategy),
            m_claimBarrier(waitStrategy), m_published(new std::atomic<sequence_t>[bufferSize]), m_nextClaimable(0) {
        ...
        ///
        /// Add a sequence barrier that prevents elements of the ring buffer
        /// from being claimed by writers until the reader has indicated the
        /// slot is available by publishing the sequence number they have
        /// finished reading. Claimed slots will never advance more than
        /// buffer_size() ahead of any of the registered claim barriers.
        ///
        /// \param barrier
        ...
        /// other threads are accessing the claim strategy concurrently with
        /// this call.
        void add_claim_barrier(sequence_barrier<WaitStrategy>& barrier) { m_claimBarrier.add(barrier); }
        ...
        /// being claimed by writers until the readers have indicated the slot
        /// is available by publishing the sequence number they have finished
        /// reading. Claimed slots will never advance more than buffer_size()
        /// ahead of any of the registered claim barriers.
        ///
        /// \param barrier
        /// The sequence barriers to add.
        /// A reference to each sequence barrier in the group is held by the
        /// claim strategy, so the caller must ensure the lifetime of the
        /// sequence barriers exceed that of the cliam strategy. This barrier
        /// must have been constructed with the same wait strategy object as the
        /// claim strategy was constructed with.
        ///
        /// \note
        /// This operation is not thread-safe and the caller must ensure that no
        /// other threads are accessing the claim strategy concurrently with
        /// this call.
        void add_claim_barrier(sequence_barrier_group<WaitStrategy>& barrier) { m_claimBarrier.add(barrier); }
        ...
        sequence_t claim_one() {
            ...
            m_claimBarrier.wait_until_published(static_cast<sequence_t>(sequence - m_bufferSize));
            return sequence;
        }
        ...
        sequence_range claim(size_t count) {
            ...
            m_claimBarrier.wait_until_published(static_cast<sequence_t>(range.last() - m_bufferSize));
            return range;
        }
        ...
        /// range of sequence numbers that were claimed. If no slots were
        /// claimed then the value is left unchanged.
        ///
        /// \return
        /// Returns \c true if any elements were claimed in which case the range
        /// of slots claimed is written to the \p range out-parameter. May claim
        /// less slots than requested if fewer slots were available. Returns \c
        /// false if no elements were claimed. The \p range out-parameter is not
        /// modified in this case.
        bool try_claim(size_t count, sequence_range& range) {
            sequence_t published = static_cast<sequence_t>(m_claimBarrier.last_published() + m_bufferSize);
            ...
            } while (!m_nextClaimable.compare_exchange_weak(sequence, static_cast<sequence_t>(sequence + count),
        ...
        ///
        /// Blocks the caller until either at least one slot was claimed or
        /// until the specified timeout period has elapsed, whichever comes
        /// first.
        ///
        /// This operation has 'acquire' memory semantics.
        ///
        ...
        /// WaitStrategy::wait_until_published().
        template <class Rep, class Period>
        bool try_claim_for(size_t count, sequence_range& range, std::chrono::duration<Rep, Period> const& timeout) {
            return try_claim_until(count, range, std::chrono::high_resolution_clock::now() + timeout);
        }
        ...
        ///
        /// \param timeoutTime
        /// The time after which the call should stop waiting for available
        /// slots.
        ///
        /// \return
        /// Returns \c true if any slots were claimed in which case the range
        ...
        bool try_claim_until(size_t count, sequence_range& range,
                             std::chrono::time_point<Clock, Duration> const& timeoutTime) {
            sequence_t published = static_cast<sequence_t>(m_claimBarrier.last_published() + m_bufferSize);
            ...
            do {
                ...
                if (diff < 0) {
                    published =
                        static_cast<sequence_t>(m_claimBarrier.wait_until_published(
            ...
            } while (!m_nextClaimable.compare_exchange_weak(sequence, static_cast<sequence_t>(sequence + reducedCount),
                                                            std::memory_order_relaxed, std::memory_order_relaxed));
        ...
        /// of the \ref single_threaded_claim_strategy::publish(sequence_t)
        /// method. As multiple threads may publish elements out of order, every
        ...
        /// Throws any exception thrown by
        /// WaitStrategy::signal_all_when_blocking().
        void publish(sequence_range const& range) {
            for (size_t i = 0, j = range.size(); i < j; ++i) { set_published(range[i]); }
            m_waitStrategy.signal_all_when_blocking();
        }
        ...
        /// The initial value passed in here on first call should be
        /// sequence_t(-1).
        ///
        /// \return
        /// The last-published sequence number.
        /// This will be equal to \p lastKnownPublished if no additional
        ...
        ///
        /// \throw std::exception
        /// Throws any exception thrown by \c
        /// WaitStrategy::wait_until_published().
        sequence_t wait_until_published(sequence_t sequence, sequence_t lastKnownPublished) const {
            ...
            for (sequence_t seq = lastKnownPublished + 1; difference(seq, sequence) <= 0; ++seq) {
                if (!is_published(seq)) {
                    std::atomic<sequence_t> const* const sequences[1] = {&m_published[seq & m_indexMask]};
        ...
        sequence_t wait_until_published(sequence_t sequence, sequence_t lastKnownPublished,
                                        std::chrono::duration<Rep, Period> const& timeout) const {
        ...
        sequence_t wait_until_published(sequence_t sequence, sequence_t lastKnownPublished,
                                        std::chrono::time_point<Clock, Duration> const& timeoutTime) const {
            assert(difference(sequence, lastKnownPublished) > 0);

            for (sequence_t seq = lastKnownPublished + 1; difference(seq, sequence) <= 0; ++seq) {
                if (!is_published(seq)) {
                    std::atomic<sequence_t> const* const sequences[1] = {&m_published[seq & m_indexMask]};
                    sequence_t result = m_waitStrategy.wait_until_published(seq, 1, sequences);
    ...
    private:
        ...
        std::unique_ptr<std::atomic<sequence_t>[]> const m_published;
