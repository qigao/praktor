...
namespace disruptorplus {
    ...
    public:
        ...
        ///
        /// \param waitStrategy
        /// The wait strategy to use for threads waiting on this sequence
        /// barrier. The constructed sequence barrier will hold a reference to
        /// this object so callers must ensure the lifetime of the wait strategy
        /// exceeds that of the sequence barrier.
        explicit sequence_barrier(WaitStrategy& waitStrategy) :
            m_waitStrategy(waitStrategy), m_pad1{0}, m_lastPublished(static_cast<sequence_t>(-1)), m_pad2{0} {}
        ...
        sequence_t last_published() const { return m_lastPublished.load(std::memory_order_acquire); }
        ...
        sequence_t wait_until_published(sequence_t sequence) const {
            ...
            std::atomic<sequence_t> const* const sequences[] = {&m_lastPublished};
            return m_waitStrategy.wait_until_published(sequence, 1, sequences);
        }

        /// \brief
        /// Block the calling thread until either the specified sequence number
        /// is published or a timeout has been exceeded.
        ///
        /// This operation synchronises with the corresponding call to publish()
        /// that publishes the returned sequence number and has 'acquire' memory
        /// semantics.
        ...
        /// published. Otherwise if <tt>difference(result, sequence) < 0</tt>
        /// then the operation has timed out.
        ///
        /// \throws std::exception
        /// May throw any exception thrown by the
        /// WaitStrategy::wait_until_published() function.
        ...
        /// \brief
        /// Block the calling thread until either the specified sequence number
        /// is published or a timeout time has been exceeded.
        ///
        /// This operation synchronises with the corresponding call to publish()
        /// that publishes the returned sequence number and has 'acquire' memory
        /// semantics.
        ...
        /// wait_until_published() overloads that wait on the specified \p
        /// sequence. This operation has 'release' memory semantics.
        ///
        /// \param sequence
