...
namespace disruptorplus {
    ...
    public:
        ...
        /// barrier group. The constructed sequence barrier will hold a
        /// reference to this object so callers must ensure the lifetime of the
        /// wait strategy exceeds that of the sequence barrier group.
        sequence_barrier_group(WaitStrategy& waitStrategy) : m_waitStrategy(waitStrategy) {}
        ...
        /// the group.
        void add(sequence_barrier<WaitStrategy> const& barrier) {
        ...
        /// If there was insufficient memory to add the sequence barriers to
        /// the group.
        void add(sequence_barrier_group<WaitStrategy> const& barrierGroup) {
            m_sequences.insert(m_sequences.end(), barrierGroup.m_sequences.begin(), barrierGroup.m_sequences.end());
        }
        ...
        sequence_t wait_until_published(sequence_t sequence) const {
            ...
            if (difference(current, sequence) >= 0) { return current; }
    ...
    private:
        ...
        std::vector<std::atomic<sequence_t> const*> m_sequences;
    };
}   // namespace disruptorplus
