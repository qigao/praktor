...
namespace disruptorplus {
    ...
    public:
        ...
        explicit ring_buffer(size_t size) : m_size(size), m_mask(size - 1), m_data(new T[size]) {
        ...
        /// modulo \ref size().
        reference operator[](sequence_t seq) { return m_data[static_cast<size_t>(seq) & m_mask]; }

        /// \copydoc ring_buffer::operator[](sequence_t)
        const_reference operator[](sequence_t seq) const { return m_data[static_cast<size_t>(seq) & m_mask]; }
