...
namespace disruptorplus {
    ...
    public:
        ...
        /// If the system does not have enough resources to perform this
        /// operation.
        static sequence_t wait_until_published(sequence_t const sequence, size_t const count,
                                               std::atomic<sequence_t> const* const sequences[]) {
            assert(count > 0);
            spin_wait spinner;
        ...
        sequence_t wait_until_published(sequence_t sequence, size_t count,
                                        std::atomic<sequence_t> const* const sequences[],
        ...
        /// \brief
        /// Wait until either all the specified sequences have at least
        ...
        sequence_t wait_until_published(sequence_t const sequence, size_t const count,
                                        std::atomic<sequence_t> const* const sequences[],
        ...
        /// \brief
        /// Notify any waiting threads that one of the sequence values has
        /// changed.
        static void signal_all_when_blocking() {
            // This is requred as part of the wait_strategy interface but does
            // nothing for the spin_wait_strategy since all waiting threads are
            // continuously checking the sequence values in a spin-wait loop.
        }
    };
}   // namespace disruptorplus
