...
namespace disruptorplus {
    ...
    /// the slots and indicates to readers it has finished writing to the slots
    /// by calling publish().
    ///
    ...
    ///
    /// Readers indicate they are finished reading from a slot in the ring
    /// buffer by publishing the sequence number of the latest item they are
    /// finished with in the 'claim barrier'. This frees the slot up for writers
    /// to subsequently claim it for future writes.
    ///
    /// \tparam WaitStrategy
    ...
    public:
        ...
        single_threaded_claim_strategy(size_t bufferSize, WaitStrategy& waitStrategy) :
            m_bufferSize(bufferSize), m_nextSequenceToClaim(0), m_lastKnownClaimableSequence(0),
        ...
        sequence_t claim_one() {
            m_claimBarrier.wait_until_published(static_cast<sequence_t>(m_nextSequenceToClaim - m_bufferSize));
        ...
        bool try_claim_for(size_t count, sequence_range& range, std::chrono::duration<Rep, Period> const& timeout) {
            if (try_claim(count, range)) { return true; }

            sequence_t claimable =
                static_cast<sequence_t>(m_claimBarrier.wait_until_published(
                                            static_cast<sequence_t>(m_nextSequenceToClaim - m_bufferSize), timeout) +
                                        m_bufferSize);
            sequence_diff_t diff = difference(claimable, m_nextSequenceToClaim);
            if (diff < 0) {
                // Timeout
        ...
        sequence_t wait_until_published(sequence_t sequence, std::chrono::duration<Rep, Period> const& timeout) const {
