...
namespace disruptorplus {
    ...
    inline sequence_diff_t difference(sequence_t a, sequence_t b) { return static_cast<sequence_diff_t>(a - b); }
    ...
    /// other.
    ///
    /// This operation implies acquire memory semantics on each of the
    /// sequences.
    ///
    /// \param count
    ...
    inline sequence_t minimum_sequence(size_t count, std::atomic<sequence_t> const* const sequences[]) {
        assert(count > 0);
        sequence_t minimum = sequences[0]->load(std::memory_order_acquire);
        for (size_t i = 1; i < count; ++i) {
            sequence_t seq = sequences[i]->load(std::memory_order_acquire);
            if (difference(seq, minimum) < 0) { minimum = seq; }
        }
        return minimum;
    }
    ...
    inline sequence_t minimum_sequence_after(sequence_t minimum, size_t count,
