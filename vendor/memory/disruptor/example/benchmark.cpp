...
private:
    static int64_t const s_freq;
};

int64_t const high_resolution_clock::s_freq = []() -> uint64_t {
...
void RunSingleThreadClaimStrategyBenchmark(size_t bufferSize, int runCount, int itemCount) {
    ...
    constexpr size_t maxLatencyCount = 10000000;
    ...
    for (int run = 0; run < runCount; ++run) {
        ...
        std::thread reader([&]() {
            ...
            while (!exit) {
                ...
                do {
                    ...
                    if (message.m_type == 0xdead) { exit = true; }
        ...
        std::thread writer([&]() {
            ...
            while (remaining > 0) {
                ...
                sequence_t const seqEnd = range.end();
    ...
    if (results.size() > 1) {
        ...
        for (size_t i = 1; i < results.size(); ++i) { assert(results[i] == firstResult); }
    }

    auto minTime = *std::min_element(times.begin(), times.end());
    auto maxTime = *std::max_element(times.begin(), times.end());

    uint64_t const totalItems = itemCount + 1;
    auto const minItemsPerSecond = (totalItems * 1000000000) / maxTime.count();
    auto const maxItemsPerSecond = (totalItems * 1000000000) / minTime.count();
    ...
    std::cout << bufferSize << ", " << writerBatchSize << ", " << itemCount << ", " << runCount << ", "
              << results.front() << ", " << minItemsPerSecond << ", " << maxItemsPerSecond << ", " << minLatency << ", "
              << avgLatency << ", " << maxLatency << std::endl;
}
...
void RunSingleThreadClaimStrategyBenchmarkVariousBatchSizes(size_t bufferSize, int runCount, int itemCount) {
    ...
    // RunSingleThreadClaimStrategyBenchmark<3, WaitStrategy>(bufferSize,
    // runCount, itemCount); RunSingleThreadClaimStrategyBenchmark<4,
    // WaitStrategy>(bufferSize, runCount, itemCount);
    // RunSingleThreadClaimStrategyBenchmark<8, WaitStrategy>(bufferSize,
    // runCount, itemCount); RunSingleThreadClaimStrategyBenchmark<16,
    // WaitStrategy>(bufferSize, runCount, itemCount);
    // RunSingleThreadClaimStrategyBenchmark<32, WaitStrategy>(bufferSize,
    // runCount, itemCount); RunSingleThreadClaimStrategyBenchmark<64,
    // WaitStrategy>(bufferSize, runCount, itemCount);
    // RunSingleThreadClaimStrategyBenchmark<128, WaitStrategy>(bufferSize,
    // runCount, itemCount); RunSingleThreadClaimStrategyBenchmark<256,
    // WaitStrategy>(bufferSize, runCount, itemCount);
    // RunSingleThreadClaimStrategyBenchmark<500, WaitStrategy>(bufferSize,
    // runCount, itemCount);
}
...
void RunMultiThreadClaimStrategyBenchmark(size_t bufferSize, int writerCount, int runCount, int itemCount) {
    ...
    size_t const maxLatencyCount = 1000000;
    std::vector<uint64_t> latencies(maxLatencyCount + 2, 0);
        ...
        std::thread reader([&]() {
            ...
            while (exitCount > 0) {
                ...
                do {
                    ...
                    auto latency = std::chrono::duration_cast<std::chrono::nanoseconds>(readTime - message.m_time);
                    ++latencies[std::max<uint64_t>(std::min<uint64_t>(latency.count(), maxLatencyCount) + 1, 0)];
                    if (message.m_type == 0xdead) { --exitCount; }
                } while (nextToRead++ != available);
                finishedReading.publish(available);
            }
            result = sum;
    ...
    int64_t const avgLatency = totalLatency / totalCount;

    uint64_t const totalItemCount = (itemCount + 1) * writerCount;
    uint64_t const minItemsPerSecond = (totalItemCount * 1000000000) / maxTimeNS.count();
    uint64_t const maxItemsPerSecond = (totalItemCount * 1000000000) / minTimeNS.count();

    std::cout << bufferSize << ", " << writerCount << ", " << writerBatchSize << ", " << itemCount << ", " << runCount
              << ", " << results.front() << ", " << minItemsPerSecond << ", " << maxItemsPerSecond << ", " << minLatency
              << ", " << avgLatency << ", " << maxLatency << std::endl;
}

template <typename WaitStrategy>
void RunMultiThreadClaimStrategyBenchmarkVariousBatchSizes(size_t bufferSize, int writerCount, int runCount,
                                                           int itemCount) {
    ...
    // RunMultiThreadClaimStrategyBenchmark<3, WaitStrategy>(bufferSize,
    // writerCount, runCount, itemCount);
    // RunMultiThreadClaimStrategyBenchmark<4, WaitStrategy>(bufferSize,
    // writerCount, runCount, itemCount);
    // RunMultiThreadClaimStrategyBenchmark<8, WaitStrategy>(bufferSize,
    // writerCount, runCount, itemCount);
    // RunMultiThreadClaimStrategyBenchmark<16, WaitStrategy>(bufferSize,
    // writerCount, runCount, itemCount);
    // RunMultiThreadClaimStrategyBenchmark<32, WaitStrategy>(bufferSize,
    // writerCount, runCount, itemCount);
    // RunMultiThreadClaimStrategyBenchmark<64, WaitStrategy>(bufferSize,
    // writerCount, runCount, itemCount);
    // RunMultiThreadClaimStrategyBenchmark<128, WaitStrategy>(bufferSize,
    // writerCount, runCount, itemCount);
    // RunMultiThreadClaimStrategyBenchmark<256, WaitStrategy>(bufferSize,
    // writerCount, runCount, itemCount);
    // RunMultiThreadClaimStrategyBenchmark<500, WaitStrategy>(bufferSize,
    // writerCount, runCount, itemCount);
}
...
void RunMultiThreadClaimStrategyBenchmarkVariousBufferSizes(int runCount, int itemCount) {
    ...
    for (size_t bufferSize = 256; bufferSize <= 1024 * 1024; bufferSize *= 8) {
        for (int writerCount = 1; writerCount < 4; ++writerCount) {
            RunMultiThreadClaimStrategyBenchmarkVariousBatchSizes<WaitStrategy>(bufferSize, writerCount, runCount,
                                                                                itemCount);
        }
    }
}
