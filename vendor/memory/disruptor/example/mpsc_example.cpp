...
int main(int argc, char* argv[]) {
    constexpr int itemCount = 500 * 1000 * 1000;
    constexpr size_t bufferSize = static_cast<size_t>(1) << 20;
    constexpr int writerBatchSize = 1;

    constexpr int writer1Count = itemCount / 2;

    spin_wait_strategy waitStrategy;
    multi_threaded_claim_strategy claimStrategy(bufferSize, waitStrategy);
    sequence_barrier finishedReading(waitStrategy);
    ...
    std::thread reader([&]() {
        ...
        while (!exit1 || !exit2) {
            sequence_t const available = claimStrategy.wait_until_published(nextToRead, nextToRead - 1);
            ...
            do {
                ...
                if (message.m_type == 0xdead) {
                    ...
                    for (int i = 0; i < 28; ++i) { sum += message.m_data[i]; }
    ...
    std::thread writer1([&]() {
        for (int i = 0; i < writer1Count;) {
            ...
            for (size_t j = 0; j < range.size(); ++j, ++i) {
                ...
                for (int k = 0; k < 28; ++k) { item.m_data[k] = (i + k) % 60; }
            }
    ...
    constexpr auto totalItemCount = itemCount + 2;

    auto const end = std::chrono::high_resolution_clock::now();
    auto const dur = (end - start);
    auto const durMS = std::chrono::duration_cast<std::chrono::milliseconds>(dur);
    auto const durNS = std::chrono::duration_cast<std::chrono::nanoseconds>(dur);
    auto const nsPerItem = durNS / totalItemCount;
    ...
    for (size_t i = 0; i < 20 && i < sortedBatchSizes.size(); ++i) {
        size_t const batchSize = sortedBatchSizes[i].second;
        size_t const batchItemCount = sortedBatchSizes[i].first;
        size_t const percentage = (100 * batchItemCount) / itemCount;
        std::cout << "#" << (i + 1) << ": " << batchSize << " item batch, " << percentage << "%, "
