#include "lockfree.hpp"                  // for BipartiteBuf
#include "spsc/bipartite_buf_impl.hpp"   // for BipartiteBuf::WriteAcquire

#include <catch2/catch_test_macros.hpp>   // for operator""_catch_sr, opera
#include <span>                           // for span
#include <stdint.h>                       // for uint8_t, int16_t, uint32_t
#include <stdlib.h>                       // for size_t, rand
#include <thread>                         // for thread
#include <utility>                        // for pair
#include <vector>                         // for vector, _Vector_const_iter
#include <xmemory>                        // for _Simple_types
#include <xtr1common>                     // for _Remove_cvref_t
#include <xutility>                       // for begin, end, equal, copy, s

TEST_CASE("spsc::BipartiteBuf - Write to the beginning", "[bb_write_beginning]") {
    lockfree::spsc::BipartiteBuf<uint8_t, 512U> bb;
    uint8_t const test_data[320] = {0xE5U};

    auto* write_location = bb.WriteAcquire(std::size(test_data));
    REQUIRE(write_location != nullptr);

    std::copy(std::begin(test_data), std::end(test_data), write_location);

    bb.WriteRelease(std::size(test_data));
...
TEST_CASE("spsc::BipartiteBuf - Try to acquire too much data", "[bb_acquire_too_much]") {
    lockfree::spsc::BipartiteBuf<uint8_t, 512U> bb;
    auto* write_location = bb.WriteAcquire(512U + rand());
    REQUIRE(write_location == nullptr);
}

TEST_CASE("spsc::BipartiteBuf - Try to acquire read with an empty buffer", "[bb_read_empty]") {
    lockfree::spsc::BipartiteBuf<uint8_t, 512U> bb;
...
TEST_CASE("spsc::BipartiteBuf - Write with overflow condition", "[bb_write_overflow]") {
    lockfree::spsc::BipartiteBuf<uint32_t, 512U> bb;
    uint32_t const test_data[320] = {0xE5A1D2C3U};

    /* Write to the start and read the data back */
    auto* write_location = bb.WriteAcquire(sizeof(test_data) / sizeof(test_data[0]));
    std::copy(std::begin(test_data), std::end(test_data), write_location);
     ...
     * beginning again */
    uint32_t const test_data2[240] = {0xA3B2C1D0U};
    write_location = bb.WriteAcquire(sizeof(test_data2) / sizeof(test_data2[0]));
    REQUIRE(write_location != nullptr);
...
TEST_CASE("spsc::BipartiteBuf - Read data written after overflow condition write", "[bb_read_after_overflow_write]") {
    lockfree::spsc::BipartiteBuf<int16_t, 512U> bb;
    int16_t const test_data[320] = {-222};
    ...
    /* Write again, this time the oveflow will trigger and should give us the
     * beginning again */
    int16_t const test_data2[240] = {-66};
    write_location = bb.WriteAcquire(sizeof(test_data2) / sizeof(test_data2[0]));
    std::copy(std::begin(test_data2), std::end(test_data2), write_location);
    ...
    uint8_t const test_data3[120] = {0xBCU};
    write_location = bb.WriteAcquire(sizeof(test_data3) / sizeof(test_data3[0]));
...
TEST_CASE("spsc::BipartiteBuf - Interleaved write and read with enough space", "[bb_interleaved_success]") {
    lockfree::spsc::BipartiteBuf<double, 512U> bb;
    double const test_data[320] = {42.4242};
    ...
    double const test_data2[120] = {-123.123};
    write_location = bb.WriteAcquire(sizeof(test_data2) / sizeof(test_data2[0]));
    REQUIRE(write_location != nullptr);
    std::copy(std::begin(test_data2), std::end(test_data2), write_location);
...
TEST_CASE("spsc::BipartiteBuf - Interleaved write and read with enough space 2", "[bb_interleaved_success2]") {
    lockfree::spsc::BipartiteBuf<char, 512U> bb;
    char const test_data[320] = {'A'};
...
TEST_CASE("spsc::BipartiteBuf - Interleaved write and read without enough space", "[bb_interleaved_fail]") {
    lockfree::spsc::BipartiteBuf<uint8_t, 512U> bb;
    uint8_t const test_data[320] = {0xE5U};
    ...
    uint8_t const test_data2[240] = {0xA3U};
    write_location = bb.WriteAcquire(sizeof(test_data2) / sizeof(test_data2[0]));
    REQUIRE(write_location == nullptr);
}
...
TEST_CASE("spsc::BipartiteBuf - Multithreaded read/write multiple", "[bb_multithread_multiple]") {
    ...
    threads.emplace_back([&]() {
        ...
        do {
            ...
            if (read_region.second) {
                read.insert(read.end(), &read_region.first[0], &read_region.first[read_region.second]);
