...
TEST_CASE("spsc::PriorityQueue - Write to empty, lowest priority and read back", "[pq_write_empty_lowest]") {
...
TEST_CASE("spsc::PriorityQueue - Write to empty, highest priority and read back", "[pq_write_empty_highest]") {
    lockfree::spsc::PriorityQueue<int16_t, 20, 3> queue;
...
TEST_CASE("spsc::PriorityQueue - Multithreaded read/write", "[pq_multithreaded]") {
    lockfree::spsc::PriorityQueue<uint64_t, 10, 4> queue;
    ...
    */
    ...
    for (size_t read_idx = 0; read_idx < read.size(); read_idx++) {
        uint64_t const read_value = read[read_idx];
        uint64_t const read_priority = read_value & ((1 << 2) - 1);
        ...
        for (size_t write_idx = 0; write_idx < written.size(); write_idx++) {
            ...
            uint64_t const written_value = written[write_idx];
            uint64_t const written_priority = written_value & ((1 << 2) - 1);
