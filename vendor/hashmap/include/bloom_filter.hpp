...
static std::size_t const bits_per_char = 0x08;   // 8 bits in 1 char(unsigned)

static unsigned char const bit_mask[bits_per_char] = {
    0x01,   // 00000001
    0x02,   // 00000010
    0x04,   // 00000100
    0x08,   // 00001000
...
public:
    bloom_parameters() :
        minimum_size(1), maximum_size(std::numeric_limits<unsigned long long int>::max()), minimum_number_of_hashes(1),
        maximum_number_of_hashes(std::numeric_limits<unsigned int>::max()), projected_element_count(10000),
        false_positive_probability(1.0 / projected_element_count), random_seed(0xA5A5A5A55A5A5A5AULL) {}
    ...
    virtual bool compute_optimal_parameters() {
        ...
        */
        ...
        while (k < 1000.0) {
            double const numerator = (-k * projected_element_count);
            double const denominator = std::log(1.0 - std::pow(false_positive_probability, 1.0 / k));

            double const curr_m = numerator / denominator;
...
public:
    ...
    bloom_filter(bloom_parameters const& p) :
    ...
    bloom_filter(bloom_filter const& filter) { this->operator=(filter); }

    inline bool operator==(bloom_filter const& f) const {
        if (this != &f) {
    ...
    inline bool operator!=(bloom_filter const& f) const { return !operator==(f); }

    inline bloom_filter& operator=(bloom_filter const& f) {
        if (this != &f) {
    ...
    inline void insert(unsigned char const* key_begin, std::size_t const& length) {
    ...
    inline void insert(T const& t) {
        ...
        insert(reinterpret_cast<unsigned char const*>(&t), sizeof(T));
    }

    inline void insert(std::string const& key) {
        insert(reinterpret_cast<unsigned char const*>(key.data()), key.size());
    }

    inline void insert(char const* data, std::size_t const& length) {
        insert(reinterpret_cast<unsigned char const*>(data), length);
    }

    template <typename InputIterator>
    inline void insert(InputIterator const begin, InputIterator const end) {
        InputIterator itr = begin;

        while (end != itr) { insert(*(itr++)); }
    }

    inline virtual bool contains(unsigned char const* key_begin, std::size_t const length) const {
        std::size_t bit_index = 0;
        std::size_t bit = 0;
        ...
        for (std::size_t i = 0; i < salt_.size(); ++i) {
            ...
            if ((bit_table_[bit_index / bits_per_char] & bit_mask[bit]) != bit_mask[bit]) { return false; }
        }
    ...
    inline bool contains(T const& t) const {
        return contains(reinterpret_cast<unsigned char const*>(&t), static_cast<std::size_t>(sizeof(T)));
    }

    inline bool contains(std::string const& key) const {
        return contains(reinterpret_cast<unsigned char const*>(key.c_str()), key.size());
    }

    inline bool contains(char const* data, std::size_t const& length) const {
        return contains(reinterpret_cast<unsigned char const*>(data), length);
    }

    template <typename InputIterator>
    inline InputIterator contains_all(InputIterator const begin, InputIterator const end) const {
        InputIterator itr = begin;
    ...
    template <typename InputIterator>
    inline InputIterator contains_none(InputIterator const begin, InputIterator const end) const {
        InputIterator itr = begin;
    ...
    inline cell_type const* table() const { return bit_table_.data(); }

    inline std::size_t hash_count() { return salt_.size(); }

protected:
    inline virtual void compute_indices(bloom_type const& hash, std::size_t& bit_index, std::size_t& bit) const {
    ...
    void generate_unique_salt() {
        ...
        */
        unsigned int const predef_salt_count = 128;

        static bloom_type const predef_salt[predef_salt_count] = {
    ...
    inline bloom_type hash_ap(unsigned char const* begin, std::size_t remaining_length, bloom_type hash) const {
        unsigned char const* itr = begin;
        unsigned int loop = 0;

        while (remaining_length >= 8) {
            unsigned int const& i1 = *(reinterpret_cast<unsigned int const*>(itr));
            itr += sizeof(unsigned int);
            unsigned int const& i2 = *(reinterpret_cast<unsigned int const*>(itr));
            itr += sizeof(unsigned int);
        ...
        if (remaining_length) {
            if (remaining_length >= 4) {
                unsigned int const& i = *(reinterpret_cast<unsigned int const*>(itr));
            ...
            if (remaining_length >= 2) {
                unsigned short const& i = *(reinterpret_cast<unsigned short const*>(itr));
...
inline bloom_filter operator&(bloom_filter const& a, bloom_filter const& b) {
...
public:
    compressible_bloom_filter(bloom_parameters const& p) : bloom_filter(p) { size_list.push_back(table_size_); }

    inline unsigned long long int size() const { return size_list.back(); }

    inline bool compress(double const& percentage) {
        ...
        unsigned long long int const new_tbl_raw_size = new_table_size / bits_per_char;
...
private:
    inline void compute_indices(bloom_type const& hash, std::size_t& bit_index, std::size_t& bit) const {
        bit_index = hash;
