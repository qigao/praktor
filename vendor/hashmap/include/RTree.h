...
#include <algorithm>
#include <assert.h>
#include <cmath>   // Include cmath for general math functions
#include <functional>
#include <limits>
#include <math.h>
#include <numbers>   // Include the C++20 numbers library for standard constants
#include <queue>
#include <stdio.h>
#include <stdlib.h>
#include <vector>
...
public:
    ...
    bool Load(char const* a_fileName);
    /// Load tree contents from stream
    bool Load(RTFileStream& a_stream);

    /// Save tree contents to file
    bool Save(char const* a_fileName);
...
protected:
    ...
    bool InsertRectRec(Branch const& a_branch, Node* a_node, Node** a_newNode, int a_level);
    bool InsertRect(Branch const& a_branch, Node** a_root, int a_level);
    Rect NodeCover(Node* a_node);
    bool AddBranch(Branch const* a_branch, Node* a_node, Node** a_newNode);
    void DisconnectBranch(Node* a_node, int a_index);
    int PickBranch(Rect const* a_rect, Node* a_node);
    Rect CombineRect(Rect const* a_rectA, Rect const* a_rectB);
    void SplitNode(Node* a_node, Branch const* a_branch, Node** a_newNode);
    ELEMTYPEREAL RectSphericalVolume(Rect* a_rect);
    ELEMTYPEREAL RectVolume(Rect* a_rect);
    ELEMTYPEREAL CalcRectVolume(Rect* a_rect);
    void GetBranches(Node* a_node, Branch const* a_branch, PartitionVars* a_parVars);
...
public:
    ...
    bool Open(char const* a_fileName, char const* mode) {
    ...
    bool OpenRead(char const* a_fileName) { return this->Open(a_fileName, "rb"); }

    bool OpenWrite(char const* a_fileName) { return this->Open(a_fileName, "wb"); }
...
RTREE_QUAL::RTree() {
    ...
    float const UNIT_SPHERE_VOLUMES[] = {
...
RTREE_TEMPLATE
RTREE_QUAL::RTree(RTree const& other) : RTree() { CopyRec(m_root, other.m_root); }
...
#ifdef _DEBUG
    for (int index = 0; index < NUMDIMS; ++index) { RTREE_ASSERT(a_min[index] <= a_max[index]); }
#endif   //_DEBUG
...
void RTREE_QUAL::CountRec(Node* a_node, int& a_count) {
    ...
    {
        for (int index = 0; index < a_node->m_count; ++index) { CountRec(a_node->m_branch[index].m_child, a_count); }
    } else   // A leaf node
    {
...
RTREE_TEMPLATE
bool RTREE_QUAL::Load(char const* a_fileName) {
...
RTREE_TEMPLATE
bool RTREE_QUAL::Save(char const* a_fileName) {
    RTFileStream stream;
...
void RTREE_QUAL::RemoveAllRec(Node* a_node) {
    ...
    {
        for (int index = 0; index < a_node->m_count; ++index) { RemoveAllRec(a_node->m_branch[index].m_child); }
    }
    FreeNode(a_node);
}
...
RTREE_TEMPLATE
bool RTREE_QUAL::InsertRectRec(Branch const& a_branch, Node* a_node, Node** a_newNode, int a_level) {
...
//
RTREE_TEMPLATE
bool RTREE_QUAL::InsertRect(Branch const& a_branch, Node** a_root, int a_level) {
    RTREE_ASSERT(a_root);
...
RTREE_TEMPLATE
bool RTREE_QUAL::AddBranch(Branch const* a_branch, Node* a_node, Node** a_newNode) {
...
RTREE_TEMPLATE
int RTREE_QUAL::PickBranch(Rect const* a_rect, Node* a_node) {
...
RTREE_TEMPLATE
typename RTREE_QUAL::Rect RTREE_QUAL::CombineRect(Rect const* a_rectA, Rect const* a_rectB) {
...
RTREE_TEMPLATE
void RTREE_QUAL::SplitNode(Node* a_node, Branch const* a_branch, Node** a_newNode) {
    RTREE_ASSERT(a_node);
    RTREE_ASSERT(a_branch);
...
ELEMTYPEREAL RTREE_QUAL::RectVolume(Rect* a_rect) {
    ...
    for (int index = 0; index < NUMDIMS; ++index) { volume *= a_rect->m_max[index] - a_rect->m_min[index]; }
...
RTREE_TEMPLATE
void RTREE_QUAL::GetBranches(Node* a_node, Branch const* a_branch, PartitionVars* a_parVars) {
    RTREE_ASSERT(a_node);
    RTREE_ASSERT(a_branch);
    ...
    for (int index = 0; index < MAXNODES; ++index) { a_parVars->m_branchBuf[index] = a_node->m_branch[index]; }
...
void RTREE_QUAL::ChoosePartition(PartitionVars* a_parVars, int a_minFill) {
    ...
    if ((a_parVars->m_count[0] + a_parVars->m_count[1]) < a_parVars->m_total) {
        ...
        for (int index = 0; index < a_parVars->m_total; ++index) {
            if (PartitionVars::NOT_TAKEN == a_parVars->m_partition[index]) { Classify(index, group, a_parVars); }
        }
    }
...
void RTREE_QUAL::InitParVars(PartitionVars* a_parVars, int a_maxRects, int a_minFill) {
    ...
    for (int index = 0; index < a_maxRects; ++index) { a_parVars->m_partition[index] = PartitionVars::NOT_TAKEN; }
}
